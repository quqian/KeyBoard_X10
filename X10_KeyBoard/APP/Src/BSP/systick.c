...
{
	timer_counter_value_config(TIMERX[1], 0);
	timer_enable(TIMERX[1]);

	while(timer_flag_get(TIMERX[1],TIMER_FLAG_UP) == RESET)
	{
		;
	}
	timer_flag_clear(TIMERX[1],TIMER_FLAG_UP);
	
	timer_disable(TIMERX[1]);
}
